...
public class PageTag<T> implements Tag {
	...
	public int doEndTag() throws JspException {
		...
		if (pagevo != null && url != null) {
			...
			if (pagevo.getCurPage() > 1) {
				sb.append("<li><a href='").append(url).append("?curPage=").append(pagevo.getCurPage() - 1).append("&pageSize=").append(this.pageSize).append(this.paramsToString())
						.append("'>&laquo;</a></li>");
			} else {
				sb.append("<li class='disabled'><a class='disabled' href='").append(url).append("?curPage=")
						.append(pagevo.getCurPage() - 1).append("&pageSize=").append(this.pageSize).append(this.paramsToString()).append("'>&laquo;</a></li>");
			}
			for (int i = (int) pagevo.getPageStart(), j = 0; i <= pagevo.getPageEnd(); i++, j++) {
				if ((pagevo.getPageStart() + j) == pagevo.getCurPage()) {
					sb.append("<li class='active'>").append("<a href='").append(url).append("?curPage=")
							.append(pagevo.getPageStart() + j).append("&pageSize=").append(this.pageSize).append(this.paramsToString()).append("'>").append(pagevo.getPageStart() + j)
							.append("</a></li>");
				} else {
			...
			if (pagevo.getCurPage() < pagevo.getTotalPage()) {
				...
				sb.append("<li class='disabled'><a class='disabled'href=").append(url).append("?curPage=")
						.append(pagevo.getTotalPage()).append("&pageSize=").append(this.pageSize).append(this.paramsToString()).append(">&raquo;</a></li>");
			}
	...
	private String paramsToString(){
		...
		for(Map.Entry<String, String[]> entry : params.entrySet()){
			...
			if(!"curPage".equals(entry.getKey())&&!"pageSize".equals(entry.getKey())){
				for(String value : values){
					strbuf.append("&").append(entry.getKey()).append("=").append(value);
				}
			}
		}
